...
def extract_offers_from_html(html: str) -> list[dict]:
    ...
    if not match:
        print(f"[ICA] Hittade inte __INITIAL_DATA__ i HTML (sidans första 500 tecken: {html[:500]!r})")
        return []
    try:
        data = json.loads(match.group(1))
    except json.JSONDecodeError as e:
        print(f"[ICA] JSON-parse-fel: {e}")
        print(f"[ICA] Rådata runt felet: {match.group(1)[max(0,e.pos-100):e.pos+100]!r}")
        return []
