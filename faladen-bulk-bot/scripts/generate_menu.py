...
- Håll instruktionsstegen korta – max 6 steg per recept, varje steg max 15 ord.
- Varorna i listan är VECKANS EXTRAPRISER – bygg recepten kring dem, inte kring fullprisvaror.

KRITISK REGEL FÖR INKÖPSLISTAN:
Inköpslistan har två separata delar:

DEL 1 – "offer_items": Varor från veckans EXTRAPRISER (det som faktiskt handlas).
- Inkludera BARA varor som finns med i erbjudandelistan du fått.
- Dessa får ett price_sek baserat på erbjudandepriset.
- Dessa räknas med i total_cost_sek.

DEL 2 – "pantry_items": Basvaror som kan behöva fyllas på.
- Inkludera ALLA övriga ingredienser från recepten (olja, pasta, ris, kryddor, lök, vitlök, burktomater osv).
- Dessa har INGET pris (price_sek saknas helt).
- Dessa räknas INTE in i total_cost_sek.
- Det är oacceptabelt om en basvarsingrediens från ett recept saknas här.

REGEL FÖR total_cost_sek:
- Räkna ENBART ihop priserna från offer_items.
- Målet är att hålla total_cost_sek under 625 kr/vecka.
...
{{
  ...
  "shopping_list": {{
    "offer_items": [
      {{ "name": "Vara från extrapris", "amount": "X kg", "store": "willys" | "ica", "price_sek": 69.90, "checked": false }}
    ],
    "pantry_items": [
      {{ "name": "Basvara", "amount": "X dl", "checked": false }}
    ]
  }},
  "total_cost_sek": 0,
